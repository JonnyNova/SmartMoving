					...
					if(!useStandard)
					{
						...
						if(isShallowDiveOrSwim && realMinPlayerSwimWaterDepth < SwimCrawlWaterBottomBorder)
						{
							...
							else
							{
								...
								resetHeightOffset();
								sp.moveEntity(0, getMaxPlayerSolidBetween(getBoundingBox().minY, getBoundingBox().maxY, 0) - getBoundingBox().minY, 0);
								isCrawling = false;
								isDiving = false;
								isSwimming = false;
								isShallowDiveOrSwim = false;
		...
		if(Config.isSmartBaseClimb() || Config.isFreeClimbingEnabled())
		{
			...
			if(Config.isFreeClimbingEnabled() && sp.fallDistance <= Config._freeClimbFallMaximumDistance.value && (!isOnLadderOrVine || Config.isFreeBaseClimb()))
			{
				...
				if(preferClimb)
				{
					...
					{
						if(!sp.isCollidedHorizontally && isAirBlock(i, j, k) && isAirBlock(i, j + 1, k))
							handsClimbing = HandsClimbing.None;
					}
	...
	public void beforeOnUpdate()
	{
		...
		if(sp.worldObj.isRemote && updateCounter < 10)
		{
			...
			for(int i=0; i<chatMessageList.size(); i++)
				if(SmartMovingComm.processBlockCode(((ChatLine)chatMessageList.get(i)).getChatComponent().getUnformattedText()))
